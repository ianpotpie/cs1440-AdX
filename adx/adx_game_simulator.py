...
class AdXGameSimulator:
    ...
    def init_agents(
        self, agent_types: List[type]
    ) -> Dict[NDaysNCampaignsAgent, CampaignBidderState]:
        states = dict()
        self.agents = []
    ...
    def generate_campaign(
        self, start_day: int, end_day: Optional[int] = None
    ) -> Campaign:
        ...
        return Campaign(
            reach=reach, target=mkt_segment, start_day=start_day, end_day=end_day
        )
    ...
    def run_ad_auctions(
        self, bid_bundles: List[BidBundle], users: List[MarketSegment], day: int
    ) -> None:
        ...
        for campaign in new_campaigns:
            ...
            for agent in self.agents:
                if campaign in agent_bids[agent]:
                    ...
                    if self.states[
                        agent
                    ].quality_score > 0 and self.is_valid_campaign_bid(
                        agent_bid, reach
                    ):
            ...
            if bids:
                ...
                if len(bids) == 1:
                    q_low = 0.0
                    all_quality_scores = [
                        self.states[agent].quality_score for agent in self.agents
                    ]
                    sorted_quality_scores = sorted(all_quality_scores)
                    for i in range(3):
                        q_low += sorted_quality_scores[
                            min(i, len(sorted_quality_scores) - 1)
                        ]
                    q_low /= 3
    ...
    def print_game_results(self) -> None:
        ...
        print("\n\t###########################################")
        for agent in sorted(
            self.agents, key=lambda a: self.states[a].profits, reverse=True
        ):
            agent_state = self.states[agent]
            print(f"\n\t### {agent.name:12} \t# {agent_state.profits:8.2f} \t###")
        print("\n\t###########################################")
    ...
    def print_final_results(
        self, total_profits: Dict[NDaysNCampaignsAgent, float], num_simulations: int
    ) -> None:
        print("\n\t################# SIMULATION RESULTS ##################")
        print("\n\t#### Agent \t\t# Profit \t###")
        print("\n\t###########################################")
        for agent in sorted(
            self.agents, key=lambda a: total_profits[a.name], reverse=True
        ):
            print(
    ...
    def run_simulation(
        self, agents: list[NDaysNCampaignsAgent], num_simulations: int
    ) -> dict[any, float]:
        ...
        for i in range(num_simulations):
            ...
            for day in range(1, self.num_days + 1):
                ...
                if day + 1 < self.num_days + 1:
                    new_campaigns = [
                        self.generate_campaign(start_day=day + 1)
                        for _ in range(self.campaigns_per_day)
                    ]
                    new_campaigns = [
                        c for c in new_campaigns if c.end_day <= self.num_days
                    ]
                ...
                for agent in self.agents:
                    ...
                    for campaign in agent_state.campaigns.values():
                        if campaign.start_day <= day <= campaign.end_day:
                            if day == campaign.end_day:
                                ...
                                effective_reach = calculate_effective_reach(
                    ...
                    if new_qs_count > 0:
                        ...
                        self.states[agent].quality_score = (1 - self.α) * self.states[
                            agent
                        ].quality_score + self.α * new_qs_val
